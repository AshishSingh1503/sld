...
import { Svg, Circle, Rect, Polygon, Ellipse, Path, Line as SvgLine } from 'react-native-svg';
...
type ShapesHubProps = {
  ...
  onStartLearn3D: () => void;
  onStartTest3D: () => void;
  onStartTransformations: () => void;
  onStartLearnAngles: () => void;
  onStartTestAngles: () => void;
};
...
const COLORS = {
  ...
  shape_green: '#2ECC71',
  shape_yellow: '#F1C40F',
  angle_acute: '#F1C40F', // Yellow
  angle_right: '#E74C3C', // Red
  angle_obtuse: '#9B59B6', // Purple
};
...
const AngleDisplay: React.FC<{ type: 'acute' | 'right' | 'obtuse'; size?: number; color?: string }> = ({ type, size = 150, color = COLORS.secondary }) => {
    const center = size / 2;
    ...
    switch (type) {
        case 'acute': angleDeg = 45; break;
        case 'right': angleDeg = 90; break;
        case 'obtuse': angleDeg = 135; break;
    }
    ...
    return (
        <Svg height={size} width={size}>
            <SvgLine x1={center} y1={center} x2={center - radius} y2={center} stroke={color} strokeWidth="5" strokeLinecap="round" />
            <SvgLine x1={center} y1={center} x2={x2} y2={y2} stroke={color} strokeWidth="5" strokeLinecap="round" />
            {type === 'right' && (
                <Path d={`M ${center - 20} ${center} L ${center - 20} ${center - 20} L ${center} ${center - 20}`} stroke={COLORS.angle_right} strokeWidth="3" fill="none" />
            )}
        </Svg>
    );
};
...
// 1. Shapes Hub Screen
const ShapesHubScreen: React.FC<ShapesHubProps> = ({ onStartLearn2D, onStartTest2D, onStartLearn3D, onStartTest3D, onStartTransformations, onStartLearnAngles, onStartTestAngles }) => (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <ModuleCard title="Learn Angles" icon="📐" description="Acute, Right, and Obtuse angles." onPress={onStartLearnAngles} />
          <ModuleCard title="Test Angles" icon="✍️" description="Classify different angles." onPress={onStartTestAngles} />
          <ModuleCard title="Transformations" icon="🔄" description="Explore flips, turns, and symmetry." onPress={onStartTransformations} />
      </ScrollView>
    </SafeAreaView>
);
...
// 6. Transformations Screen
const TransformationsScreen: React.FC<ScreenProps> = ({ onBack }) => {
    const shapes = ['triangle', 'rectangle', 'square', 'circle'];
    const [shapeIndex, setShapeIndex] = useState(0);
    
    const rotationAnim = useRef(new Animated.Value(0)).current;
    const reflectionAnim = useRef(new Animated.Value(1)).current;
    
    const [rotationValue, setRotationValue] = useState(0);
    const [isReflected, setIsReflected] = useState(false);

    const rotate = () => {
        const newRotation = rotationValue + 45;
        setRotationValue(newRotation);
        Animated.timing(rotationAnim, { toValue: newRotation, duration: 300, useNativeDriver: true }).start();
    };

    const reflect = () => {
        const newIsReflected = !isReflected;
        setIsReflected(newIsReflected);
        Animated.timing(reflectionAnim, { toValue: newIsReflected ? -1 : 1, duration: 300, useNativeDriver: true }).start();
    };
    
    const changeShape = () => {
        const newIndex = (shapeIndex + 1) % shapes.length;
        setShapeIndex(newIndex);
        setRotationValue(0);
        rotationAnim.setValue(0);
        setIsReflected(false);
        reflectionAnim.setValue(1);
    };

    const animatedStyle = {
        transform: [
            { rotate: rotationAnim.interpolate({ inputRange: [0, 360], outputRange: ['0deg', '360deg'] }) },
            { scaleX: reflectionAnim },
        ],
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                {onBack && <TouchableOpacity style={styles.backButton} onPress={onBack}><Text style={styles.backButtonText}>← Back</Text></TouchableOpacity>}
                <Text style={styles.instructionText}>Explore Flips and Turns</Text>
            </View>
            <View style={styles.shapeCanvas}>
                <View style={styles.symmetryLine} />
                <Animated.View style={[styles.transformContainer, animatedStyle]}>
                    <ShapeDisplay shape={shapes[shapeIndex]} color={COLORS.secondary} size={150}/>
                </Animated.View>
            </View>
            <View style={styles.optionsFooter}>
                <AppButton title="Rotate ↻" onPress={rotate} />
                <AppButton title="Reflect ↔" onPress={reflect} />
                <AppButton title="New Shape" onPress={changeShape} />
            </View>
        </SafeAreaView>
    );
};

// 7. Learn Angles Screen
const LearnAnglesScreen: React.FC<ScreenProps> = ({ onBack }) => {
    const angleTypes: ('acute' | 'right' | 'obtuse')[] = ['acute', 'right', 'obtuse'];
    const [index, setIndex] = useState(0);

    const next = () => setIndex(prev => (prev + 1) % angleTypes.length);
    const prev = () => setIndex(prev => (prev - 1 + angleTypes.length) % angleTypes.length);
    
    const currentAngle = angleTypes[index];
    const description = {
        acute: 'An acute angle is smaller than a right angle.',
        right: 'A right angle is a perfect corner, like a square.',
        obtuse: 'An obtuse angle is bigger than a right angle.'
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                {onBack && <TouchableOpacity style={styles.backButton} onPress={onBack}><Text style={styles.backButtonText}>← Back</Text></TouchableOpacity>}
                <Text style={styles.instructionText}>This is an {currentAngle} angle</Text>
            </View>
            <View style={styles.shapeCanvas}>
                <AngleDisplay type={currentAngle} size={250} color={COLORS[`angle_${currentAngle}`]} />
                <Text style={styles.descriptionText}>{description[currentAngle]}</Text>
            </View>
            <View style={styles.optionsFooter}>
                <AppButton title="⬅️ Prev" onPress={prev} />
                <AppButton title="Next ➡️" onPress={next} />
...
// 8. Test Angles Screen
const TestAnglesScreen: React.FC<ScreenProps> = ({ onBack }) => {
    const angleTypes: ('acute' | 'right' | 'obtuse')[] = ['acute', 'right', 'obtuse'];
    const [problemType, setProblemType] = useState<'acute' | 'right' | 'obtuse'>('right');
    const [feedback, setFeedback] = useState<{message: string; type: 'correct' | 'incorrect'} | null>(null);

    const generateNewQuestion = () => {
        const newType = angleTypes[Math.floor(Math.random() * angleTypes.length)];
    ...
    const handleAnswer = (selectedType: string) => {
        if (selectedType === problemType) {
            ...
            setFeedback({ type: 'incorrect', message: `That's an ${problemType} angle.` });
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                {onBack && <TouchableOpacity style={styles.backButton} onPress={onBack}><Text style={styles.backButtonText}>← Back</Text></TouchableOpacity>}
                <Text style={styles.instructionText}>What type of angle is this?</Text>
            </View>
            <View style={styles.shapeCanvas}>
                <AngleDisplay type={problemType} size={250} />
            </View>
            {feedback && <FeedbackIndicator message={feedback.message} type={feedback.type} />}
            <View style={styles.optionsFooter}>
                {angleTypes.map(type => (
                    <AppButton key={type} title={type.charAt(0).toUpperCase() + type.slice(1)} onPress={() => handleAnswer(type)} disabled={!!feedback} style={{backgroundColor: COLORS[`angle_${type}`]}}/>
                ))}
            </View>
        </SafeAreaView>
    );
...
const ShapesModule: React.FC = () => {
  const [screen, setScreen] = useState<'main' | 'learn2d' | 'test2d' | 'learn3d' | 'test3d' | 'transform' | 'learnAngles' | 'testAngles'>('main');
  ...
  if (screen === 'transform') return <TransformationsScreen onBack={() => setScreen('main')} />;
  if (screen === 'learnAngles') return <LearnAnglesScreen onBack={() => setScreen('main')} />;
  if (screen === 'testAngles') return <TestAnglesScreen onBack={() => setScreen('main')} />;
  ...
  return (
    <ShapesHubScreen
      ...
      onStartTransformations={() => setScreen('transform')}
      onStartLearnAngles={() => setScreen('learnAngles')}
      onStartTestAngles={() => setScreen('testAngles')}
    />
  );
};
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: COLORS.primary, paddingVertical: 15, paddingHorizontal: 20, borderRadius: 30, alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 5, elevation: 3, marginHorizontal: 5 },
