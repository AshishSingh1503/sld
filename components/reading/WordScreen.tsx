...
const WordScreen = ({ goBack, userClass }: WordScreenProps) => {
  ...
  const loadClassWords = async () => {
    try {
      ...
      if (classWords.length > 0) {
        setWord(contentService.getRandomItem(classWords));
      }
    } catch (error) {
      console.error('Error loading class words:', error);
  ...
  const pickRandomWord = () => {
    if (words.length > 0) {
      const randomWord = contentService.getRandomItem(words);
      setWord(randomWord);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.loadingText}>Loading Class {userClass} words...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.classInfo}>Class {userClass} Words</Text>

      {/* Word Card */}
      <View style={styles.card}>
        <Text style={styles.word}>{word || 'No words available'}</Text>
      ...
      {/* Next Word Button */}
      <TouchableOpacity onPress={pickRandomWord} style={styles.nextButton}>
